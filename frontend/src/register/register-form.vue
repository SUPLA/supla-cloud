<template>
  <div class="create-form">
    <h1 v-title class="page-title">{{ $t('Create an account') }}</h1>
    ...
    <form class="register-form" @submit.prevent="submit()">
      ...
      <div :class="['form-group', {'has-error': passwordTouched && errorPassword}]">
        ...
        <input id="password" v-model="password" type="password" class="form-control" @blur="passwordTouched = true" />
        <div class="help-block for-error-only">
      ...
      <div :class="['form-group', {'has-error': errorPasswordConfirm}]">
        ...
        <input id="password-confirm" v-model="confirmPassword" type="password" class="form-control" @blur="passwordConfirmTouched = true" />
        <div class="help-block for-error-only">
      ...
      <div :class="['form-group', {'has-error': errorRegulations}]">
        <regulations-checkbox v-if="regulationsAcceptRequired" v-model="regulationsAgreed" @input="regulationsTouched = true" />
        <div class="help-block for-error-only">
      ...
      <div v-if="captchaEnabled">
        <invisible-recaptcha id="registerRecaptcha" :sitekey="captchaSiteKey" :callback="checkCaptcha" :disabled="isBusy" :form-valid="formIsValid">
          <div>
            <span v-if="!isBusy">
              {{ $t('Create an account') }}
            </span>
  ...
  export default {
    ...
    methods: {
      ...
      submit() {
        ...
        this.isBusy = true;
        api
          .post('register-account', data, {skipErrorHandler: [400, 409]})
