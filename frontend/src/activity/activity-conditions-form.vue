<template>
  <div>
    ...
    <DateRangePicker v-model="activeDateRange" :label-date-start="$t('Active from')" :label-date-end="$t('Active to')" @input="onChanged()" />
    <div class="form-group text-center">
      <label>
        <label class="checkbox2 checkbox2-grey">
          <input v-model="useWorkingSchedule" type="checkbox" @change="onChanged()" />
          {{ $t('Use activity schedule') }}
        </label>
      </label>
    </div>
    <transition-expand>
      <WeekScheduleSelector v-if="useWorkingSchedule" v-model="activeHours" class="narrow mode-1-green" @input="onChanged()" />
    </transition-expand>
    <DaytimeActivityConditions v-model="activityConditions" class="mt-4" @input="onChanged()" />
  </div>
</template>
    ...
    computed: {
      modelValue() {
        return {
          ...
          activeHours: this.useWorkingSchedule
            ? mapValues(this.activeHours, (hours) => {
