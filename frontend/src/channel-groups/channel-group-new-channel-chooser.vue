  ...
  <flipper :flipped="!!addingNewChannel">
    <template #front>
      <square-link class="clearfix pointer black">
        <a class="valign-center text-center" @click="addingNewChannel = true">
          <span>
    ...
    <template #back>
      <square-link class="clearfix pointer black not-transform">
        <span class="valign-center text-center">
          <span>
            <div v-if="userData.limits.channelPerGroup <= channelGroup.channels.length" @click="addingNewChannel = false">
              <i class="pe-7s-close-circle"></i>
              {{ $t('Limit has been exceeded') }}
            </div>
            <div v-else-if="channelsToChoose && channelsToChoose.length === 0" @click="addingNewChannel = false">
            ...
            <form v-else @submit.prevent="addChannel()">
              ...
              <div class="form-group">
                <button class="btn btn-default pull-left" type="button" @click="addingNewChannel = false">
                  {{ $t('Cancel') }}
                </button>
                <button class="btn btn-green pull-right" :disabled="!newChannel" type="submit">
                  {{ $t('Add') }}
                </button>
              </div>
            </form>
