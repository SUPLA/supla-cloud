<template>
  <div>
    <loading-cover :loading="!devices">
      <div v-show="devices && devices.length" class="container">
        <device-filters @filter-function="filterFunction = $event" @compare-function="compareFunction = $event"></device-filters>
      </div>
      <square-links-grid
        v-if="(compareFunction && filteredDevices && filteredDevices.length) || (showPossibleDevices && !devices.length)"
        :count="filteredDevices.length + (showPossibleDevices ? possibleDevices.length : 0)"
      >
        <div v-for="device in filteredDevices" :key="device.id" :ref="'device-tile-' + device.id">
          <device-tile :device="device"></device-tile>
        </div>
        <template v-if="showPossibleDevices">
          <div v-for="possibleDevice in possibleDevices" :key="'possible' + possibleDevice.title">
