<template>
  <div v-title="$t('Account')" class="account-page">
    <animated-svg id="user-account-bg" :file="withBaseUrl('assets/img/user-account-bg.svg', false)"></animated-svg>
        ...
        <transition name="fade">
          <div v-if="user" class="user-account">
            ...
            <div class="form-group text-center">
              <a class="btn btn-default" @click="changingNotifications = true">{{ $t('Account notifications') }}</a>
              <a class="btn btn-default" @click="showingLimits = true">{{ $t('Show my limits') }}</a>
            </div>
            <div class="text-center">
              <a class="btn btn-red-outline btn-xs" @click="deletingAccount = true">{{ $t('Delete my account') }}</a>
            </div>
          </div>
        </transition>
      </loading-cover>
    ...
    <div v-if="user">
      <account-opt-out-notifications-modal
        v-if="changingNotifications"
        :user="user"
        @cancel="closeOptOutNotificationsModal()"
      ></account-opt-out-notifications-modal>
      <account-limits-modal v-if="showingLimits" :user="user" @confirm="showingLimits = false"></account-limits-modal>
      <account-delete-modal v-if="deletingAccount" :user="user" @cancel="deletingAccount = false"></account-delete-modal>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      ...mapState(useFrontendConfigStore, {
