<template>
  <div class="container">
    <loading-cover :loading="authAttempts === undefined">
      <table v-if="authAttempts" class="table table-striped">
        <thead>
          <tr>
            <th>{{ $t('Operation') }}</th>
...
<script>
  ...
  export default {
    ...
    mounted() {
      api
        .get('users/current/audit', {params: {events: ['AUTHENTICATION_SUCCESS', 'AUTHENTICATION_FAILURE', 'PASSWORD_RESET', 'PASSWORD_CHANGED']}})
        .then((response) => {
