  ...
  <div>
    <div v-if="channel.config.virtualChannelConfig.type === 'ENERGY_PRICE_FORECAST'">
      ...
      <dl v-if="channel.config.virtualChannelConfig.energyField === 'pdgsz'" class="mt-2">
        ...
        <dt>
          <div v-for="level in levels" :key="level.badge" class="pdgsz-legend d-flex" :style="{'border-color': level.color}">
            <span class="mr-2"
              ><span class="badge" :style="{background: level.color}">{{ level.badge }}</span></span
            >
...
<script setup>
  ...
  const levels = [
    ...
    {
      label: 'RECOMMENDED SAVING. Schedule energy-intensive household activities for other hours.',
      color: '#f2c433',
      badge: '2',
    }, // i18n
    {
      label: 'REQUIRED LIMITATION. Limit your electricity consumption to the essential minimum.',
      color: '#e42313',
      badge: '3',
    }, // i18n
  ];
