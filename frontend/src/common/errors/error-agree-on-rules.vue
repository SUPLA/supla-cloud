<template>
  <loading-cover class="container text-center" :loading="loading">
    <h1 class="nocapitalize">{{ $t('The Terms and Conditions have changed') }}</h1>
    <i class="pe-7s-note2 error-page-icon" style="font-size: 160px"></i>
    ...
    <div class="form-group">
      <a class="btn btn-yellow" @click="disagree()">
        <i class="pe-7s-back"></i>
        {{ $t('I do not agree, get me out of here') }}
      </a>
      <button class="btn btn-green" type="submit" @click="agree()">
...
<script>
  ...
  export default {
    ...
    methods: {
      agree() {
        this.loading = true;
        api.patch('users/current', {action: 'agree:rules'}).finally(() => window.location.assign(window.location.toString()));
      },
      disagree() {
