<template>
  <pending-changes-page :header="$t('OCR settings')" dont-set-page-title :is-pending="hasPendingChanges" @cancel="cancelChanges()" @save="saveOcrSettings()">
    <loading-cover :loading="!latestImageDetails && !latestImageError">
      <transition-expand>
        <div v-if="conflictingConfig" class="row">
      ...
      <div class="form-group text-center">
        <label>
          <label class="checkbox2 checkbox2-grey">
            <input v-model="ocrSettings.enabled" type="checkbox" @change="onChange()" />
      ...
      <transition-expand>
        <div v-if="ocrSettings.enabled">
          <div class="form-group">
            ...
            <TimeIntervalSlider v-model="ocrSettings.photoIntervalSec" :min="60" seconds class="mt-5" @input="onChange()" />
          </div>
          ...
          <div v-if="ocrSettings.availableLightingModes.length">
            ...
            <div class="form-group">
              ...
              <NumberInput v-model="ocrSettings.lightingLevel" :min="1" :max="100" suffix="%" class="mt-2" @update:modelValue="$emit('change')" required />
            </div>
          </div>
          ...
          <div v-else-if="latestImageDetails">
            <div class="form-group">
              ...
              <div class="d-flex align-items-center">
                ...
                <div>
                  <button type="button" class="btn btn-white text-nowrap" :disabled="editingPhoto" @click="editingPhoto = true">
                    {{ $t('Edit area crop') }}
                  </button>
                </div>
              </div>
              <div class="ocr-photo-crop-area">
                <OcrPhotoCrop v-model="ocrSettings.photoSettings" :editable="editingPhoto" :image-base64="latestImageDetails.image" @input="onChange()" />
              </div>
            </div>
          </div>

          <div class="text-center">
            <button class="btn btn-white" type="button" :disabled="takingPhoto" @click="takePhotoNow()">
              {{ $t('Take a photo now') }}
            </button>
  ...
  export default {
    ...
    methods: {
      ...
      fetchLatestImage() {
        ...
        api
          .get(`integrations/ocr/${this.subject.id}/images/latest`, {skipErrorHandler: true})
      ...
      takePhotoNow() {
        ...
        api
          .patch(`channels/${this.subject.id}/settings`, {action: 'ocr:takePhoto'})
          .then(() => successNotification(this.$t('Command has been sent to the device. Try refreshing the photo in a while.')))
