<template>
  <div>
    ...
    <table class="table">
      ...
      <tbody>
        <tr v-for="(image, $index) in images" :key="image.id">
          ...
          <td>
            <img v-if="image.imageCropped" :src="`data:image/png;base64,${image.imageCropped}`" class="ocr-image" />
            <span v-else class="label label-danger">{{ $t('Error') }}</span>
          </td>
          <td class="text-center">
            <span v-if="image.processedAt">
              <span :class="['label', image.measurementValid === null ? 'label-default' : image.measurementValid ? 'label-success' : 'label-warning']">{{
                image.resultMeasurement || $t('Error')
              }}</span>
              <span v-if="image.measurementValid === false && $index === 0" class="ml-2">
                <a v-tooltip.bottom="$t('Does it look good?')" class="small text-warning" @click="fixingLastMeasurement = true">
                  <fa icon="wrench" />
                </a>
              </span>
            </span>
            <span v-else class="label label-default">{{ $t('Waiting') }}</span>
          </td>
        </tr>
      </tbody>
    </table>
    ...
    <modal-confirm
      v-if="fixingLastMeasurement"
      class="modal-warning"
      :header="$t('Everyone makes mistakes!')"
      :loading="markingLastMeasurement || isFetching"
      @confirm="
        markLastMeasurementValid()
          .then(fetchOcrMeasurements)
          .finally(() => (fixingLastMeasurement = false))
      "
    ...
    >
      <div>
        ...
        <p v-if="latestMeasurement.imageCropped" class="text-center mb-3">
          <img :src="`data:image/png;base64,${latestMeasurement.imageCropped}`" class="ocr-image ocr-image-bigger" />
        </p>
        <p>
          {{
            $t(
              'You can mark this reading manually as valid. This action WILL NOT immediately update the counter value, but it should improve the future readings for your counter.'
            )
          }}
        </p>
...
<script setup>
  ...
  const {execute: markLastMeasurementValid, isFetching: markingLastMeasurement} = useSuplaApi(`channels/${props.subject.id}/settings`, {
    immediate: false,
  }).patch({
