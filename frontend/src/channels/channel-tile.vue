<template>
  <square-link :class="`clearfix pointer with-label ${backgroundColor}`" @click="$emit('click')">
    <router-link :to="linkSpec">
      ...
      <dl v-if="model.caption" class="ellipsis">
        <dd></dd>
      ...
      <dl v-if="device && !model.isVirtual" class="ellipsis">
        <dd>{{ $t('Device') }}</dd>
        <dt>{{ device.name }}</dt>
      </dl>
      <dl class="ellipsis">
      ...
      <dl v-if="location" class="ellipsis">
        <dd>{{ $t('Location') }}</dd>
      ...
      <div v-if="hasActionTrigger" class="square-link-label-left">
