...
export class IndexedDbMeasurementLogsStorage {
  ...
  async fetchDenseLogs(afterTimestamp, beforeTimestamp, aggregationMethod = 'minute') {
    ...
    if (keyFunc) {
      ...
      const finalLogs = aggregatedLogs.map(this.chartStrategy.aggregateLogs).map((log) => {
  ...
  async init() {
    return api.get(`channels/${this.channel.id}/measurement-logs?order=DESC&limit=1000&logsType=${this.getLogsType()}`).then(async ({body: logItems}) => {
      if (logItems.length) {
        logItems.reverse();
        if (this.hasSupport) {
  ...
  async fetchOlderLogs(progressCallback, somethingDownloaded = false) {
    ...
    if (oldestLog) {
      ...
      return api
        .get(`channels/${this.channel.id}/measurement-logs?order=DESC&limit=2500&beforeTimestamp=${beforeTimestamp}&logsType=${this.getLogsType()}`)
  ...
  async storeLogs(logs) {
    ...
    logs
      .map((log) => this.chartStrategy.fixLog(log))
      .forEach(async (log) => {
