<template>
  <div>
      ...
      <div v-if="storage.isReady">
        <div class="form-group">
          ...
          <div class="radio d-flex justify-content-center">
            <label class="mx-3"><input v-model="downloadConfig.dateRange" type="radio" value="selected" /> {{ $t('Selected') }}</label>
            <label class="mx-3"><input v-model="downloadConfig.dateRange" type="radio" value="all" /> {{ $t('All') }}</label>
          </div>
        </div>
        ...
        <div class="form-group">
          ...
          <div class="radio d-flex justify-content-center">
            <label class="mx-3"><input v-model="downloadConfig.format" type="radio" value="csv" /> CSV</label>
            <label class="mx-3"><input v-model="downloadConfig.format" type="radio" value="ods" /> ODS</label>
            <label class="mx-3"><input v-model="downloadConfig.format" type="radio" value="xlsx" /> XLSX</label>
            <label class="mx-3"><input v-model="downloadConfig.format" type="radio" value="html" /> HTML</label>
          </div>
        </div>
        ...
        <transition-expand>
          <div v-if="downloadConfig.format === 'csv'" class="form-group">
            ...
            <div class="radio d-flex justify-content-center">
              <label class="mx-3"><input v-model="downloadConfig.separator" type="radio" value="," /> {{ $t('Comma') }} <code>,</code></label>
              <label class="mx-3"><input v-model="downloadConfig.separator" type="radio" value=";" /> {{ $t('Colon') }} <code>;</code></label>
              <label class="mx-3"><input v-model="downloadConfig.separator" type="radio" value="tab" /> {{ $t('Tab') }} <code>\t</code></label>
            </div>
          </div>
        </transition-expand>
        ...
        <transition-expand>
          <div v-if="supportsIncrementalLogs" class="form-group">
            ...
            <div class="radio text-center">
              <label class="mx-3"
                ><input v-model="downloadConfig.transformation" type="radio" value="cumulative" />
                {{ $t('Counter') }}
                <span class="small">({{ $t('values as seen on counter') }})</span>
              </label>
              <label class="mx-3"
                ><input v-model="downloadConfig.transformation" type="radio" value="none" />
                {{ $t('Incremental') }}
        ...
        <div class="text-center mt-4">
          <a v-if="!downloading" class="btn btn-default" @click="download()">
    ...
    <div v-else class="text-center">
      <a :href="withDownloadAccessToken(`/api/channels/${channel.id}/measurement-logs-csv?`)" class="btn btn-default">
        <fa icon="download" class="mr-1" />
        {{ $t('Download the history of measurement') }}
      </a>
    </div>
...
<script>
  ...
  import {channelTitle, withDownloadAccessToken} from '@/common/filters';
  ...
  export default {
    ...
    computed: {
      ...
      supportsIncrementalLogs() {
        return (
          !CUSTOM_CHART_MODES.includes(this.chartMode) &&
          [
    ...
    methods: {
      ...
      async downloadFile(rows) {
        ...
        const sheetName = channelTitle(this.channel)
          .replace(/[^0-9a-z]/gi, '_')
          .replace(/_+/g, ' ')
          .substr(0, 30);
