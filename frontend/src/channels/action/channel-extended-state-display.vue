<template>
  <div v-if="source && source.state.extendedState && sourceStates.length > 0" :class="{disconnected: !isConnected && !condensed, condensed}">
    <div v-if="!isConnected" class="alert alert-warning small py-1 px-5 mb-2">
      ...
      states.push(
        ...[
          ...
          {label: 'Operating time', key: 'operatingTime', format: duration}, // i18n
          {
            ...
            key: 'lightSourceLifespan',
            format: (v) => `${v} ${i18n.t('hours')}`,
          }, // i18n
        ...
        ...[
          {
            label: 'Power supply',
            key: 'batteryPowered',
            format: (v) => (v ? i18n.t('battery') : i18n.t('mains')),
          }, // i18n
