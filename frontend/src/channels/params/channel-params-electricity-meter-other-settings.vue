  ...
  <div>
    <dl v-if="channel.config.availableCTTypes && channel.config.availableCTTypes.length > 0">
      ...
      <dt>
        ...
        <SimpleDropdown v-model="channel.config.usedCTType" :options="channel.config.availableCTTypes" @input="$emit('change')">
    ...
    <dl v-if="channel.config.availablePhaseLedTypes && channel.config.availablePhaseLedTypes.length > 0">
      ...
      <dt>
        ...
        <SimpleDropdown v-slot="{value}" v-model="channel.config.usedPhaseLedType" :options="channel.config.availablePhaseLedTypes" @input="$emit('change')">
    ...
    <transition-expand>
      <div v-if="channel.config.usedPhaseLedType === 'VOLTAGE_LEVEL'">
        <dl>
          ...
          <dt>
            <NumberInput v-model="lowV" :min="0" :max="highV || 1000" :precision="2" suffix="V" @update:modelValue="$emit('change')" required />
          </dt>
        </dl>
        <dl>
          <dd>{{ $t('High threshold') }}</dd>
          <dt>
            <NumberInput v-model="highV" :min="lowV || 0" :max="1000" :precision="2" suffix="V" @update:modelValue="$emit('change')" required />
          </dt>
        </dl>
        <div class="small">
          <div>
...
<script setup>
  ...
  watch(
    () => props.channel.config.usedPhaseLedType,
    () => {
      if (props.channel.config.usedPhaseLedType === 'VOLTAGE_LEVEL') {
