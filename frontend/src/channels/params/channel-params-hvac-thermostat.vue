<template>
  <div>
    ...
    <transition-expand>
      <div v-if="!channel.config.mainThermometerChannelId" class="alert alert-warning mt-2">
    ...
    <dl v-if="channel.config.masterThermostatAvailable && canDisplaySetting('masterThermostatChannelId')">
      ...
      <dt>
        <channels-id-dropdown
          ...
          @input="
            masterThermostatChannelIdJustChanged = true;
            $emit('change');
          "
        ></channels-id-dropdown>
      </dt>
    ...
    <transition-expand>
      <div v-if="masterThermostatChannelIdJustChanged && channel.config.masterThermostatChannelId" class="alert alert-info my-2">
    ...
    <AccordionRoot>
      <AccordionItem
        ...
        title-i18n="Thermometers configuration"
      >
        <dl v-if="canDisplaySetting('mainThermometerChannelId')">
          ...
          <dt>
            <channels-id-dropdown
              ...
              @input="$emit('change')"
            ></channels-id-dropdown>
            <SameDifferentThanMasterThermostat
              :channel="channel"
              :master-thermostat="masterThermostat"
              setting="mainThermometerChannelId"
              @change="$emit('change')"
            />
          </dt>
        </dl>
        <dl v-if="canDisplaySetting('auxThermometerChannelId')">
          ...
          <dt>
            ...
            <SameDifferentThanMasterThermostat
              :channel="channel"
              :master-thermostat="masterThermostat"
              setting="auxThermometerChannelId"
              @change="$emit('change')"
            />
          </dt>
        </dl>
          ...
          <div v-if="channel.config.auxThermometerChannelId && canDisplaySetting('auxThermometerType')">
            <dl>
              ...
              <dt>
                ...
                <SimpleDropdown
                  v-slot="{value}"
                  v-model="channel.config.auxThermometerType"
        ...
        <transition-expand>
          <div
            v-if="
              ['FLOOR', 'WATER', 'GENERIC_HEATER', 'GENERIC_COOLER'].includes(channel.config.auxThermometerType) &&
              canDisplaySetting('auxMinMaxSetpointEnabled') &&
              channel.config.auxThermometerType !== 'DISABLED'
            "
          >
            <dl class="wide-label">
              <dd>
              ...
              <dt class="text-center">
                <toggler
                  v-model="channel.config.auxMinMaxSetpointEnabled"
        ...
        <div v-if="channel.config.temperatureControlType && canDisplaySetting('temperatureControlType')">
          <dl>
            ...
            <dt>
              ...
              <SimpleDropdown
                v-slot="{value}"
                v-model="channel.config.temperatureControlType"
      ...
      <AccordionItem
        v-if="
          canDisplayAnySetting('antiFreezeAndOverheatProtectionEnabled') ||
          (channel.config.antiFreezeAndOverheatProtectionEnabled && freezeHeatProtectionTemperatures.length)
        "
        :title-i18n="
          freezeHeatProtectionTemperatures.length === 2
            ? $t('Anti-freeze and overheat protection')
            : freezeHeatProtectionTemperatures[0].name === 'heatProtection'
              ? $t('Overheat protection')
              : $t('Anti-freeze protection')
        "
      >
        <dl v-if="canDisplaySetting('antiFreezeAndOverheatProtectionEnabled')">
          ...
          <dt class="text-center">
            <toggler
              v-model="channel.config.antiFreezeAndOverheatProtectionEnabled"
      ...
      >
        ...
        <dl v-if="channel.config.heatOrColdSourceSwitchAvailable && canDisplaySetting('heatOrColdSourceSwitchChannelId')">
          ...
          <dt>
            ...
            <SameDifferentThanMasterThermostat
              :channel="channel"
              :master-thermostat="masterThermostat"
              setting="heatOrColdSourceSwitchChannelId"
              @change="$emit('change')"
            />
          </dt>
        </dl>
        <div v-if="channel.config.availableAlgorithms.length > 0">
          <dl v-if="canDisplaySetting('usedAlgorithm')">
            ...
            <dt>
              ...
              <SimpleDropdown
                v-slot="{value}"
                v-model="channel.config.usedAlgorithm"
          ...
          <transition-expand>
            <div v-if="algorithmHelpShown" class="well small text-muted p-2 display-newlines">
        ...
        <dl v-if="canDisplaySetting('minOnTimeS')" class="wide-label">
          <dd>
        ...
        <dl v-if="canDisplaySetting('minOffTimeS')" class="wide-label">
          <dd>
        ...
        <dl v-if="canDisplaySetting('outputValueOnError')" class="wide-label">
          <dd>
          ...
          <dt>
            <select
              ...
              @change="$emit('change')"
            >
              <option v-for="possibleValue in possibleOutputValueOnErrorValues" :key="possibleValue.value" :value="possibleValue.value">
        ...
        <transition-expand>
          <div v-if="outputValueHelpShown" class="well small text-muted p-2 display-newlines">
            {{ $t('thermostatOutputValue_help') }}
          </div>
        </transition-expand>
        <dl v-if="canDisplaySetting('temperatureSetpointChangeSwitchesToManualMode') && !channel.config.masterThermostatChannelId" class="wide-label">
          <dd>{{ $t('Temperature setpoint change switches to manual mode') }}</dd>
          <dt class="text-center">
            <toggler
              v-model="channel.config.temperatureSetpointChangeSwitchesToManualMode"
      ...
      <AccordionItem v-if="canDisplayAnySetting('localUILock') && channel.config.localUILockingCapabilities" title-i18n="Device interface">
        <dl>
          <dd>{{ $t('Lock type') }}</dd>
          <dt>
  ...
  export default {
    ...
    computed: {
      ...
      histeresisTemperatures() {
        const temps = [this.availableTemperatures.find((t) => t.name === 'histeresis')];
