<template>
  <div>
    <div v-if="channel.config.waitingForConfigInit" class="alert alert-warning m-0 mt-3">
    ...
    <div v-else>
      ...
      <ChannelParamsBinaryFilteringTime v-if="channel.config.filteringTimeMs !== undefined" :channel="channel" @change="$emit('change')" />
      ...
      <component :is="additionalChannelParamsComponent" :channel="channel" @change="$emit('change')" @save="$emit('save')"></component>
      <ChannelParamsIntegrationsSettings v-if="channel.config.googleHome || channel.config.alexa" :subject="channel" @change="$emit('change')" />
    </div>
  </div>
</template>
...
<script>
  ...
  import ChannelParamsControllingthegatewaylock from './channel-params-controllingthegatewaylock.vue';
  ...
  import ChannelParamsOpeningsensorRollershutter from './channel-params-openingsensor-rollershutter.vue';
