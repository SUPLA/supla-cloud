<template>
  <div>
    <dl>
      <dd>{{ $t('Value added') }}</dd>
      <dt v-tooltip="channel.hasPendingChanges && $t('Save or discard configuration changes first.')" class="text-center">
        <a class="btn btn-default btn-block btn-wrapped" :class="{disabled: channel.hasPendingChanges}" @click="changeInitialValues()">
          {{ $t('Set value added') }}
        </a>
      </dt>
    </dl>
    <modal-confirm
      v-if="settingInitialValues"
...
<script>
  import ChannelParamsMeterInitialValuesMode from './channel-params-meter-initial-values-mode.vue';
  import ChannelParamsElectricityMeterInitialValue from '@/channels/params/channel-params-electricity-meter-initial-value.vue';
