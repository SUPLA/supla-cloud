<template>
  <div v-if="dependencies.length > 0" class="details-page-block">
...
<script setup>
  ...
  const dependencyLabel = (dep) => {
    ...
    if (
      ['masterThermostatChannelId', 'pumpSwitchChannelId', 'heatOrColdSourceSwitchChannelId'].includes(dep.role) &&
      props.channel.config[dep.role] !== dep.channelId
    ) {
      dir = 'dest';
    }
