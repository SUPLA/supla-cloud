<template>
  <div class="container">
    ...
    >
      <div v-if="subject.config.masterThermostatChannelId" class="row">
      ...
      <div v-else>
        ...
        <div v-if="subject.config.altWeeklySchedule && subject.config.heatingModeAvailable && subject.config.coolingModeAvailable" class="text-center mb-5">
          <a :class="['btn mx-2', editingMode === 'weeklySchedule' ? 'btn-orange' : 'btn-default']" @click="editingMode = 'weeklySchedule'">
            <IconHeating />
            {{ $t('Heating mode') }}
          </a>
          <a :class="['btn mx-2', editingMode === 'altWeeklySchedule' ? 'btn-blue' : 'btn-default']" @click="editingMode = 'altWeeklySchedule'">
            <IconCooling />
            {{ $t('Cooling mode') }}
          </a>
        </div>
        ...
        <ThermostatProgramsConfigurator
          v-model="editingSchedule.programSettings"
          :subject="subject"
          :default-program-mode="editingMode === 'weeklySchedule' ? 'HEAT' : 'COOL'"
          @editing="editingPrograms = $event"
          @input="hasPendingChanges = true"
          @program-chosen="(programNo) => (currentProgram = programNo)"
        />

        <WeekScheduleSelector v-model="quarters" :selection-mode="currentProgram" quarters class="week-schedule-selector-thermostat" />
      </div>
    </pending-changes-page>
  </div>
</template>
