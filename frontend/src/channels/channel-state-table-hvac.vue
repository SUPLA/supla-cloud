  ...
  <div class="channel-state-table-hvac">
    <div class="d-flex justify-content-center align-items-center">
      ...
      <span class="arrow-container">
        <fa v-if="state.heating" icon="angle-double-up" fixed-width class="arrow heating" />
        <fa v-else-if="state.cooling" icon="angle-double-down" class="arrow cooling" />
        <fa v-else-if="state.mode !== 'OFF'" icon="angle-double-right" class="arrow" />
      </span>
      <span v-if="state.mode !== 'OFF'" class="small">
        ...
        <span v-if="state.weeklyScheduleTemporalOverride" v-tooltip.bottom="$t('Temporal override')" class="ml-2">
    ...
    <div v-if="(state.heating || state.cooling) && state.partially" class="small">
      ...
      <div class="progress">
        <div
          :class="['progress-bar', state.heating ? 'progress-bar-danger' : 'progress-bar-info']"
          role="progressbar"
          :style="{width: `${state.partially}%`}"
        ></div>
      </div>
    </div>
  </div>
