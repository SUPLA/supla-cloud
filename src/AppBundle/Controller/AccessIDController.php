<?php
/*
 src/AppBundle/Controller/AccessIDController.php
...
use AppBundle\Form\Type\AccessIdType;
use AppBundle\Form\Type\AssignType;
...
class AccessIDController extends Controller
{
	...
	private function getAccessIdDetails($id) {
		...
		if ( $aid !== null ) {
			...
			return $this->get('templating')->render('AppBundle:AccessID:aiddetails.html.twig',
					array('accessid' => $aid,
    ...
    public function listAction()
    {
    	...
    	return $this->render('AppBundle:AccessID:aidlist.html.twig', 
    			             array('accessids' => $user->getAccessIds(),
    ...
    public function newAction()
    {
    	...
    	if ( $user->getLimitLoc() > 0 
    	    ...
    	    $this->get('session')->getFlashBag()->add('error', array('title' => 'Stop', 'message' => 'Access identifier limit has been exceeded'));
    		...
    		if ( $aid !== null ) {
    			...
    			$this->get('session')->getFlashBag()->add('success', array('title' => 'Success', 'message' => 'New access identifier created'));
    ...
    public function itemDeleteAction($id)
    {
    	...
    	$this->get('session')->getFlashBag()->add('warning', array('title' => 'Information', 'message' => 'Access identifier has been deleted'));
    	return $this->redirectToRoute("_aid_list");
    }   
    ...
    public function assignLocAction(Request $request, $id)
    {
    	...
    	$form = $this->createForm(new AssignType(),
    			null,
    			array('cancel_url'=>$this->generateUrl('_aid_list'))
    			);
    ...
    public function ajaxAssignLocList(Request $request, $id)
    {
    	...
    	if ( $aid !== null ) {
    		...
    		$form = $this->createForm(new AssignType(),
    				null,
    				array('cancel_url'=>$this->generateUrl('_aid_list'),
    					  'action'=>$this->generateUrl('_loc_assignloc', array('id'=>$aid->getId()))
    				)
    		);
    		 
    		$html = $this->get('templating')->render('AppBundle:AccessID:assignloc.html.twig',
    ...
    public function ajaxSetEnabled(Request $request, $id, $enabled)
    {
    	...
    	return $this->ajaxItemEdit($aid,
    			$this->get('translator')->trans('Access identifier has been '.($enabled == '1' ? 'enabled' : 'disabled')),
    ...
    private function ajaxItemSet(Request $request, $id, $caption, $field) {
    	...
    	return $this->ajaxItemEdit($aid,
    			$this->get('translator')->trans($field.' has been changed'),
    			null
    	);
    }
