  ...
  <div class="centered-form login-form">
    ...
    <div class="form-group form-group-lg login-password">
      <span class="input-group">
        ...
        <input v-model="password" type="password" :placeholder="$t('Password')" name="_password" class="form-control" />
      </span>
    </div>
    <div>
      <slot></slot>
    ...
    <div class="form-group text-right">
      <button type="submit" class="btn btn-green btn-lg" :disabled="authenticating" @click="$emit('input', {username: username, password: password})">
    ...
    <transition name="fade">
      <div v-if="error === 'sessionExpired'" class="error session-expired">
        <strong>{{ $t('Your session has been expired.') }}</strong>
        {{ $t('Please log in again to proceed.') }}
      </div>
    </transition>
    <transition name="fade">
      <div v-if="error === 'locked'" class="error locked">
        <strong>{{ $t('Your account has been locked.') }}</strong>
        {{ $t('Please wait a moment before the next login attempt.') }}
      </div>
    </transition>
    <transition name="fade">
      <div v-if="error === 'disabled'" class="error disabled">
        <p>
          <strong>{{ $t('Your account has not been confirmed yet.') }}</strong>
        </p>
    ...
    <transition name="fade">
      <div v-if="frontendConfig.maintenanceMode" class="error">
        <maintenance-warning></maintenance-warning>
      </div>
    </transition>
    <transition name="fade">
      <router-link v-if="error == 'error'" to="/forgotten-password" class="error">
        <strong>{{ $t('Forgot your password?') }}</strong>
        {{ $t('Don’t worry, you can always reset your password via email. Click here to do so.') }}
      </router-link>
    </transition>
    <div v-if="!oauth && !frontendConfig.maintenanceMode && frontendConfig.accountsRegistrationEnabled" class="additional-buttons">
      <router-link to="/forgotten-password" class="btn btn-white btn-wrapped">
