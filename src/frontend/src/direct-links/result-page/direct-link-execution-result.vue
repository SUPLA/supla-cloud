  ...
  <div v-if="directLink && directLink.id === linkId" class="container text-center">
    <div v-if="failureReason">
      ...
      <div v-if="failureReason == 'directLinkExecutionFailureReason_invalidActionParameters'">
        ...
        <p>
          {{ $t('This direct link requires further parameters in order to be executed properly. See the examples below to get an idea of what you might do.') }}
        </p>
        <div style="font-size: 1.3em">
          <div v-if="['REVEAL_PARTIALLY', 'SHUT_PARTIALLY'].includes(action)">
            <div>
              <code>{{ currentUrl }}?percentage=40</code>
            </div>
            <div>
              <code>{{ currentUrl }}?percentage=60</code>
            </div>
          </div>
          <div v-if="action === 'SET_RGBW_PARAMETERS'">
            <div v-if="['DIMMER', 'DIMMERANDRGBLIGHTING'].indexOf(directLink.subject.function.name) !== -1">
              <div>
                <code>{{ currentUrl }}?brightness=40</code>
              </div>
              <div>
                <code>{{ currentUrl }}?brightness=100</code>
              </div>
            </div>
            <div v-if="['RGBLIGHTING', 'DIMMERANDRGBLIGHTING'].indexOf(directLink.subject.function.name) !== -1">
              <div>
                <code>{{ currentUrl }}?color=0xFF6600</code>
              </div>
              <div>
                <code>{{ currentUrl }}?color_brightness=40&color=16711935</code>
              </div>
              <div>
                <code>{{ currentUrl }}?color_brightness=60&color=0x00FF00</code>
              </div>
              <div>
                <code>{{ currentUrl }}?color=random</code>
              </div>
            </div>
          </div>
          <div v-if="action === 'SET'">
            <div v-if="directLink.subject.function.name.match(/^DIGIGLASS.+/)">
              <div>
                <code>{{ currentUrl }}?transparent=1,2</code>
              </div>
              <div>
                <code>{{ currentUrl }}?opaque=2</code>
              </div>
              <div>
                <code>{{ currentUrl }}?transparent=1,2&opaque=0,3</code>
              </div>
            </div>
          </div>
          <div v-if="action === 'COPY'">
            <div>
              <code>{{ currentUrl }}?sourceChannelId=123</code>
            </div>
          </div>
          <div v-if="action === 'HVAC_SET_TEMPERATURE'">
            <div>
              <code>{{ currentUrl }}?temperature=21</code>
            </div>
          </div>
          <div v-if="action === 'HVAC_SET_TEMPERATURES'">
            <div>
              <code>{{ currentUrl }}?temperatureHeat=21&temperatureCool=2300</code>
            </div>
          </div>
        </div>
      </div>
