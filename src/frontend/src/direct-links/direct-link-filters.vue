  ...
  <div class="grid-filters">
    <btn-filters
      ...
      :filters="[
        ...
        {label: $t('ID'), value: 'id'},
        {label: $t('Last used'), value: 'lastUsed'},
      ]"
      @input="$emit('filter')"
    ></btn-filters>
    <btn-filters
  ...
  export default {
    ...
    methods: {
      matches(directLink) {
        ...
        if (this.search) {
          const searchString = latinize([directLink.id, directLink.caption].join(' ')).toLowerCase();
      ...
      compare(a, b) {
        if (this.sort === 'lastUsed') {
          return DateTime.fromISO(b.lastUsed || '2000-01-01T00:00:00').diff(DateTime.fromISO(a.lastUsed || '2000-01-01T00:00:00')).milliseconds;
