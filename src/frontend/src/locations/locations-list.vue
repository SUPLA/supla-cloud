...
<script setup>
  ...
  import LocationTile from '@/locations/location-tile.vue';
  ...
  const filtersDef = ref({
    sort: [
      ...
      {label: 'ID', value: 'id'},
      {label: 'No of devices', value: 'noOfDevices'}, // i18n
    ],
    enabled: [
      {label: 'All', value: 'all'},
  ...
  const filteredItems = computed(() => {
    ...
    } else if (f.sort === 'noOfDevices') {
      out.sort((a, b) => b.relationsCount.ioDevices - a.relationsCount.ioDevices);
    } else {
