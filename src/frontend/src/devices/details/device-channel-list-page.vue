<template>
  <div>
    <loading-cover :loading="!deviceChannels">
      <div v-show="deviceChannels && deviceChannels.length" class="container">
        <channel-filters :has-device="true" @filter-function="filterFunction = $event" @compare-function="compareFunction = $event"></channel-filters>
      </div>
      <div v-if="compareFunction && deviceChannels && deviceChannels.length">
        <div v-if="filteredChannels.length">
          <div v-for="channelsGroup in channelsBySubDevice" :key="channelsGroup[0].subDeviceId">
            <div v-if="channelsGroup[0].subDeviceId > 0" class="container">
              <SubdeviceDetails :channels="channelsGroup" />
            </div>
            <square-links-grid :count="channelsGroup.length">
              <div v-for="channel in channelsGroup" :key="channel.id">
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      filteredChannels() {
        const filteredChannels = this.deviceChannels.filter(this.filterFunction).filter((channel) => !channel.config.hideInChannelsList);
