<template>
  <div>
    <div v-if="device.config.homeScreenContentAvailable && device.config.homeScreenContentAvailable.length" class="form-group">
    ...
    <div v-if="hasOffDelay" class="form-group">
      ...
      <div>
        ...
        <SimpleDropdown v-slot="{value}" v-model="homeScreenOffMode" :options="availableOffDelayTypes">
          {{ $t('homeScreenContent_offMode_' + value) }}
        </SimpleDropdown>
      </div>
    </div>
    <transition-expand>
      <div v-if="homeScreenOffMode !== 'DISABLE' && hasOffDelay" class="form-group mt-2">
        <label>{{ $t('Automatic front panel turn off after') }}</label>
        <div class="mt-3 mb-6">
          <FormSlider
            v-model="offDelay"
            tooltip="always"
            :data="homeScreenOffPossibleDelays"
