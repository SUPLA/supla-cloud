<template>
  <div v-tooltip="disabledReason">
    <button class="btn btn-default btn-block btn-wrapped" type="button" :disabled="!!disabledReason" @click="setDeviceTime()">
    ...
    >
      <div class="form-group">
        <input v-model="deviceTime" type="datetime-local" class="form-control" />
      </div>
    </modal-confirm>
  </div>
</template>
    ...
    methods: {
      setDeviceTime(deviceTime) {
        if (deviceTime) {
          ...
          this.deviceTime = DateTime.now().startOf('second').toISO({includeOffset: false, suppressMilliseconds: true});
        }
      },
    },
    computed: {
