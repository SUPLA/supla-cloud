<template>
  <div>
    <a v-if="channel.function.id" class="btn btn-link" @click="openIconDialog()">
      {{ $t('Change icon') }}
    </a>

    <modal-confirm
      v-if="choosing"
      :header="addingNewIcon ? $t('Add a new icon') : $t('Select icon')"
      :cancellable="!addingNewIcon"
      :confirmable="!addingNewIcon"
      @cancel="choosing = addingNewIcon = false"
      @confirm="choose(selectedIcon)"
    >
      <loading-cover :loading="!icons.length">
        <div v-if="icons.length">
          <channel-user-icon-creator
            v-if="addingNewIcon || editingIcon"
            :icon="editingIcon"
            :model="channel"
          ...
          <div v-else class="d-flex icons-list flex-wrap justify-content-center">
            <a v-for="icon in icons" :key="icon.id" :class="{active: selectedIcon.id === icon.id}" @click="selectedIcon = icon">
              <channel-user-icon-preview v-if="icon.function" :icon="icon" />
              <function-icon v-else width="100" :model="channel.function" :config="channel.config" :alternative="icon.index" />
            </a>
            <a class="valign-center" @click="addingNewIcon = true">
...
<script>
  import ChannelUserIconCreator from './channel-user-icon-creator.vue';
  ...
  export default {
    components: {
      LoadingCover,
      ModalConfirm,
      ChannelUserIconPreview,
      FunctionIcon,
      ChannelUserIconCreator,
    },
    props: ['channel'],
    data() {
      return {
    ...
    methods: {
      ...
      buildIcons() {
        ...
        if (this.channel) {
          api.get('user-icons?function=' + this.channel.function.name).then((response) => {
