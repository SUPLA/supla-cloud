<template>
  <div>
    ...
    <div v-if="oldestLog && newestLog" class="history-date-range-picker d-flex my-3">
      <div class="d-flex flex-column form-group mr-3 justify-content-end">
        <button type="button" class="btn btn-default" :disabled="oldestLog.date_timestamp * 1000 + 1000 >= currentMinTimestamp" @click="panTime(-1)">
          <fa icon="chevron-left" />
        </button>
      </div>
      <DateRangePicker v-model="dateRange" :min="minDate" class="flex-grow-1" :label-date-start="$t('From')" :label-date-end="$t('To')" />
      <div class="d-flex flex-column justify-content-end form-group ml-3">
        <div class="d-flex">
          <button class="btn btn-default" type="button" :disabled="newestLog.date_timestamp * 1000 <= currentMaxTimestamp" @click="panTime(1)">
    ...
    <h3 v-if="denseLogs.length === 0" class="text-center my-5">{{ $t('Your chart is being drawn...') }}</h3>
    <div ref="bigChart"></div>
    <div
      v-if="
        channel.functionId === ChannelFunction.ELECTRICITYMETER &&
        denseLogs.length > 0 &&
        !['voltageHistory', 'currentHistory', 'powerActiveHistory'].includes(chartMode)
      "
    >
...
<script>
  ...
  import ChannelMeasurementsHistorySummaryTableElectricityMeter from '@/channels/history/channel-measurements-history-summary-table-electricity-meter.vue';
  ...
  export default {
    components: {
      ...
      DateRangePicker,
      ChannelMeasurementsPredefinedTimeRanges,
    },
    props: {
      channel: Object,
      chartMode: String,
    ...
    created() {
      this.setTimeRange = debounce(({afterTimestampMs, beforeTimestampMs}) => {
        ...
        minTimestamp = minTimestamp.set({
          seconds: 0,
          minutes: Math.floor(minTimestamp.get('minute') / 10) * 10,
        });
        let maxTimestamp = DateTime.fromMillis(beforeTimestampMs);
        maxTimestamp = maxTimestamp.set({
          seconds: 0,
          minutes: Math.ceil(minTimestamp.get('minute') / 10) * 10,
        });
    ...
    methods: {
      ...
      formatPointLabel(pointTimestampMs, nextPointTimestampMs, prevPointTimestampMs) {
        if (this.aggregationMethod === 'hour') {
          let datetimeLabel = DateTime.fromSeconds(pointTimestampMs / 1000)
            .startOf('hour')
            .toLocaleString(DateTime.DATETIME_MED);
          if (nextPointTimestampMs) {
            datetimeLabel +=
              ' - ' +
              DateTime.fromSeconds(pointTimestampMs / 1000)
                .endOf('hour')
                .toLocaleString(DateTime.DATETIME_MED);
          }
          ...
          return DateTime.fromSeconds(pointTimestampMs / 1000).toLocaleString({
            year: 'numeric',
            month: 'long',
          });
        } else {
      ...
      rerenderBigChart() {
        if (this.denseLogs && this.denseLogs.length) {
          ...
          this.bigChart.updateOptions({...merge(newChartOptions, this.chartStrategy.chartOptions.call(this)), series}, false, false);
