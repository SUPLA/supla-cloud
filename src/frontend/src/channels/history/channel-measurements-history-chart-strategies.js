...
export const CHART_TYPES = {
  ...
  HUMIDITYANDTEMPERATURE: {
    ...
    series: function (allLogs) {
      ...
      const humiditySeries = allLogs.map((item) => ({
        x: item.date_timestamp * 1000,
        y: item.humidity,
      }));
      const series = [
  ...
  IC_GASMETER: {
    ...
    series: function (allLogs) {
      const calculatedValues = allLogs.map((item) => ({
        x: item.date_timestamp * 1000,
        y: item.calculated_value,
      }));
    ...
    adjustLogs: (logs) => {
      ...
      for (let i = 1; i < logs.length; i++) {
        ...
        if (log.counter >= previousLog.counter * 0.9) {
          // .9 for reset misdetections
        ...
        return enabledPhases.map((phaseNo) => {
          ...
          return {
            ...
            data: allLogs.map((item) => ({
              x: item.date_timestamp * 1000,
              y: item[`phase${phaseNo}_${this.chartMode}`],
            })),
          };
        });
      }
        ...
        CHART_TYPES.ELECTRICITYMETER.allAttributesArray().forEach((attributeName) => {
          if (Object.prototype.hasOwnProperty.call(log, attributeName)) {
            if (log[attributeName] === null) {
              ...
              let skipThisLog = false;
              if (log[attributeName] < -latestState[attributeName] * 0.1) {
  ...
  GENERAL_PURPOSE_METER: {
    ...
    series: function (allLogs) {
      const calculatedValues = allLogs.map((item) => ({
        x: item.date_timestamp * 1000,
        y: item.counterReset ? null : item.value,
      }));
      return [{name: this.$t('Value'), data: calculatedValues}];
    },
    fixLog: (log) => {
    ...
    adjustLogs: (logs, channel) => {
      ...
      for (let i = 1; i < logs.length; i++) {
        ...
        if (counterType === 'ALWAYS_INCREMENT') {
          if (log.value >= previousLog.value - Math.abs(0.1 * previousLog.value)) {
            // 0.1 for reset misdetections
