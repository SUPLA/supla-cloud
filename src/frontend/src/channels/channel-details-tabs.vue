<template>
  <div id="channel-details-tabs" class="channel-details-tabs details-tabs">
    <div v-if="availableTabs.length" class="container">
      <div class="form-group">
        <ul class="nav nav-tabs">
          <li v-for="tabDefinition in availableTabs" :key="tabDefinition.id" :class="{active: $route.name === tabDefinition.route}">
    ...
    <RouterView v-if="tabVisible" :subject="channel" :channel="channel" />
  </div>
</template>
  ...
  export default {
    ...
    mounted() {
      ...
      if (this.availableTabs.length) {
        const isDefaultChannelRoute = this.$route.name === 'channel';
        const isDefaultTab = this.$route.name === this.availableTabs[0].route;
        const isAvailableTabRequested = this.availableTabs.map((t) => t.route).filter((r) => this.$route.name.startsWith(r)).length > 0;
