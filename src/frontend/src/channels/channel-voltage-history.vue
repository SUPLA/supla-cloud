<template>
  <div>
    <loading-cover :loading="!logs">
      <div v-if="logs && logs.length > 0" class="container">
        <div class="text-right mb-3">
          <a :href="withDownloadAccessToken(`/api/channels/${channel.id}/measurement-logs-csv?logsType=voltage&`)" class="btn btn-default mx-1">{{
          ...
          <DropdownMenu class="d-inline-block">
            ...
            <ul class="dropdown-menu dropdown-menu-right">
              <li v-for="phaseNo in enabledPhases" :key="phaseNo">
        ...
        <div class="text-center">
          <label class="checkbox2">
            <input v-model="daysWithIssuesOnly" type="checkbox" @change="selectVisibleStat()" />
            {{ $t('Hide days without aberrations') }}
          </label>
        </div>
        <div class="d-flex voltage-day-stats-container">
          <div v-for="date in statsToShow" :key="date.dayFormatted">
            <a
              v-if="date !== statsToShow[0] || !fetching"
              :class="['voltage-day-stat', `gradient-${chooseGradient(date.secTotalAboveOrBelow, maxSecTotal)}`, {selected: selectedDay === date.dayFormatted}]"
              @click="selectDay(date)"
            >
              <p class="day">{{ date.day.toLocaleString({month: 'numeric', day: 'numeric'}) }}</p>
              <p v-if="date.secTotalAboveOrBelow" class="sec" :title="$t('Number of aberrations')">
                <fa :icon="faArrowDown()" class="below-threshold-indicator" />
                {{ date.countBelowTotal }}
                <fa :icon="faArrowUp()" class="above" />
                {{ date.countAboveTotal }}
              </p>
              <p v-else class="sec">
        ...
        <div class="day-parts mb-3">
          ...
          >
            ...
            <p v-if="v.secTotalAboveOrBelow" class="sec" :title="$t('Number of aberrations')">
              <fa :icon="faArrowDown()" class="below-threshold-indicator" />
              {{ v.countBelowTotal }}
              <fa :icon="faArrowUp()" class="above-threshold-indicator" />
              {{ v.countAboveTotal }}
            </p>
            <p v-else class="sec">
              <fa :icon="faCheckCircle()" class="text-success" />
        ...
        <div class="voltage-logs-list-container">
          <div v-if="selectedDayViolations && selectedDayViolations[selectedDayPart].violations.length" class="voltage-logs-list">
            <div v-for="log in selectedDayViolations[selectedDayPart].violations" :key="log.date_timestamp" class="voltage-log panel panel-default">
  ...
  export default {
    ...
    methods: {
      ...
      fetchLogs() {
        ...
        return api
          .get(
            `channels/${this.channel.id}/measurement-logs?logsType=voltage&limit=1000&order=DESC&offset=${this.loadedLogsCount}&behforeTimestamp=${this.beforeTimestamp}`
          )
          .then(({body: logItems}) => {
