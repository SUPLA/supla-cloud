        ...
        methods: {
          deviceTitle,
            ...
            changeFunction(newFunction, safe = true) {
                ...
                api.put(`channels/${this.id}` + (safe ? '?safe=1' : ''), channel, {skipErrorHandler: [409]})
            ...
            saveChanges: throttle(function (safe = true) {
                this.loading = true;
                return api.put(`channels/${this.id}${safe ? '?safe=1' : ''}`, this.channel, {skipErrorHandler: [409]})
                    .then(response => extendObject(this.channel, response.body))
            ...
            changeLocation(location, safe = true) {
                ...
                return api.put(`channels/${this.id}${safe ? '?safe=1' : ''}`, request, {skipErrorHandler: [409]})
                    .then(() => {
