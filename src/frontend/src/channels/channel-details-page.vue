<template>
  <page-container :error="error">
        ...
        <div class="d-flex mt-3">
          <div class="flex-grow-1">
            ...
            <h4>
              ...
              <span v-if="!channel.isVirtual"> {{ $t('Channel No') }}: {{ channel.channelNumber }}, </span>
              {{ $t('ID') }}: {{ channel.id }}
            </h4>
          </div>
          <div>
        ...
        <div class="row">
            ...
            <div class="details-page-block">
              ...
              <div class="hover-editable hovered">
                <form @submit.prevent="saveChanges()">
                  <VirtualChannelInfo v-if="channel.isVirtual" :channel="channel" />
                  <dl>
                    <dd>{{ $t('Function') }}</dd>
                    <dt v-if="!channel.isVirtual" v-tooltip="hasPendingChanges && $t('Save or discard configuration changes first.')" class="text-center">
                      <a class="btn btn-default btn-block btn-wrapped" :class="{disabled: hasPendingChanges}" @click="changingFunction = true">
                        ...
                        <span v-if="channel.function.id" class="small">
                          {{ $t('Change function') }}
                        </span>
                  ...
                  <dl>
                    ...
                    <dt>
                      <input v-model="channel.caption" type="text" class="form-control text-center" :placeholder="$t('Default')" @keydown="updateChannel()" />
                    </dt>
                  </dl>
                  ...
                  <channel-params-form :channel="channel" @save="saveChanges()" @change="updateChannel()"></channel-params-form>
                  <transition-expand>
                    <ConfigConflictWarning v-if="configConflictDetected" class="mt-3 mb-0" @refresh="refreshChannelConfig()" />
                  </transition-expand>
                  <transition-expand>
                    <div v-if="!configConflictDetected && hasPendingChanges" class="text-center mt-3">
                      <a class="btn btn-grey mx-1" @click="cancelChanges()">
                        <i class="pe-7s-back"></i>
                        {{ $t('Cancel changes') }}
                      </a>
                      <button class="btn btn-white mx-1" type="submit">
          ...
          <div class="col-md-4 col-sm-6">
            ...
            <div class="details-page-block">
              ...
              <div
                v-tooltip.bottom="
                  (hasPendingChanges && $t('Save or discard configuration changes first.')) ||
                  (channel.inheritedLocation && $t('Channel is assigned to the I/O device location'))
                "
                class="form-group"
              >
                <SquareLocationChooser
                  v-model="channel.location"
              ...
              <div class="text-center">
                <a v-if="!channel.inheritedLocation && !hasPendingChanges" @click="changeLocation(null)">
          ...
          <div class="col-md-4 col-sm-6">
            <div class="details-page-block">
              ...
              <div class="text-center">
                <function-icon :model="channelsStore.all[channel.id] || channel" width="100" :flexible-width="true"></function-icon>
                <div v-if="channelFunctionIsChosen">
                  <span v-if="hasPendingChanges" v-tooltip.bottom="$t('Save or discard configuration changes first.')">
                    {{ $t('Change icon') }}
                  </span>
                  <channel-alternative-icon-chooser v-else :channel="channel" @change="saveChanges()"></channel-alternative-icon-chooser>
                </div>
                <channel-state-table v-if="channelFunctionIsChosen && !loading" class="py-3" :channel="channel"></channel-state-table>
    ...
    <channel-action-executor-modal v-if="executingAction" :subject="channel" @close="executingAction = false"></channel-action-executor-modal>

    <channel-function-edit-modal
      v-if="changingFunction"
      :channel="channel"
      :loading="loading"
      @cancel="changingFunction = false"
      @confirm="changeFunction($event)"
    ></channel-function-edit-modal>
    ...
    <dependencies-warning-modal
      ...
      @confirm="changeFunction(changeFunctionConfirmationObject.newFunction, false)"
    ></dependencies-warning-modal>
    ...
    <dependencies-warning-modal
      ...
      @confirm="changeLocation(changeLocationConfirmationObject.newLocation, false)"
    ></dependencies-warning-modal>
    ...
    <modal v-if="sleepingDeviceWarning" :header="$t('Device might be sleeping')" @confirm="sleepingDeviceWarning = false">
...
<script>
  ...
  import DeviceTile from '@/devices/list/device-tile.vue';
  ...
  import ChannelActionExecutorModal from './action/channel-action-executor-modal.vue';
  ...
  export default {
    ...
    methods: {
      ...
      fetchChannel() {
        ...
        this.channelRequest()
          .then((response) => {
            ...
            this.channel.enabled = !!this.channel.function.id;
            this.hasPendingChanges = this.loading = false;
            this.channel.configBefore = deepCopy(response.body.config);
          })
      ...
      changeFunction(newFunction, safe = true) {
        ...
        channel.functionId = newFunction.id;
        channel.altIcon = 0;
        channel.userIconId = null;
        this.loading = true;
        this.changeFunctionConfirmationObject = undefined;
        api
          .put(`channels/${this.id}` + (safe ? '?safe=1' : ''), channel, {skipErrorHandler: [409]})
      ...
      changeLocation(location, safe = true) {
        ...
        return api
          .put(`channels/${this.id}${safe ? '?safe=1' : ''}`, request, {skipErrorHandler: [409]})
          .then(() => {
