<template>
  <div>
    <form v-if="editingPrograms" @submit.stop.prevent="submitProgramsEdit()">
      <div class="thermostat-program-buttons editing">
        <div
          v-for="program in editingPrograms"
        ...
        >
          ...
          <div v-if="program.settings.mode === 'HEAT' || autoModeAvailable" class="form-group form-group-sm my-1">
          ...
          <div v-if="program.settings.mode === 'COOL' || autoModeAvailable" class="form-group form-group-sm my-1">
            <span class="input-group">
              <span class="input-group-addon">
                <IconCooling />
              </span>
    ...
    <div v-else>
      <div class="thermostat-program-buttons">
        <div
          ...
          :key="program.programNo"
          :class="[`text-center mx-3 thermostat-program-button thermostat-program-button-${program.programNo}`, {chosen: program.chosen}]"
        ...
        >
          <div v-if="program.programNo > 0">
            ...
            <div>
              ...
              <span v-else-if="program.settings.mode === 'NOT_SET'"> ? </span>
              <span v-else>
                <IconHeating />
                {{ program.settings.setpointTemperatureHeat }}&deg;C &hyphen;
                <IconCooling />
                {{ program.settings.setpointTemperatureCool }}&deg;C
              </span>
            </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      initFromValue() {
        this.programs = Object.values(
          mapValues(this.value, (settings, programNo) => {
