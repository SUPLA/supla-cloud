<template>
  <div>
    <div class="panel-group panel-accordion hovered" role="tablist" aria-multiselectable="true">
      <div
        v-for="(trigger, index) in channel.config.actionTriggerCapabilities"
        :key="index"
        :class="'panel panel-' + (channel.config.actions[trigger] ? 'success' : 'default')"
      >
        <div :id="'heading' + trigger" class="panel-heading" role="tab">
          <div class="panel-title" @click="toggleExpand(trigger)">
            <div class="d-flex">
              <a role="button" class="text-inherit flex-grow-1" :aria-expanded="!!expanded[trigger]" :aria-controls="'collapse' + trigger">
                {{ $t('actionTrigger_' + trigger) }}
              </a>
              <div>
                <span v-if="disablesLocalOperation(trigger)" class="text-monospace disable-local-operation-warning">!</span>
              </div>
            </div>
          </div>
        </div>
        <div :id="'collapse' + trigger" class="panel-collapse" role="tabpanel" :aria-labelledby="'heading' + trigger">
          <transition-expand>
            <div v-if="expanded[trigger]" class="panel-body">
              <p v-if="disablesLocalOperation(trigger)" class="small text-muted">
                {{
                  $t(
              ...
              <action-trigger-single-action-selector
                v-model="channel.config.actions[trigger]"
                :channel="channel"
                :trigger="trigger"
                @input="$emit('change')"
              ></action-trigger-single-action-selector>
