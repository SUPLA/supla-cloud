    ...
    <div class="form-group">
      ...
      >
        ...
        <div v-if="subject && subject.ownSubjectType !== 'other'" class="mt-3">
          <channel-action-chooser v-model="action" :subject="subject" :always-select-first-action="true" @input="(a) => onActionChange(a)" />
        </div>
      </subject-dropdown>
    </div>

    <button v-if="value" type="button" class="btn btn-default btn-block mt-3" @click="clearAction()">
...
<script>
  ...
  import ActionTriggerOtherActionsDropdown from '@/channels/action-trigger/action-trigger-other-actions-dropdown.vue';
      ...
      onValueChanged() {
        if (this.value?.subjectType) {
          if (this.value.subjectType === ActionableSubjectType.OTHER) {
            ...
            api.get(subjectEndpointUrl(this.value)).then((response) => {
