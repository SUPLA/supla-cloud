<template>
  <loading-cover :loading="uploading">
    ...
    <div v-show="mode === 'light'" class="form-group">
      <div class="row">
        <div v-for="(possibleState, stateIndex) in possibleStates" :key="stateIndex" :class="'col-sm-' + 12 / possibleStates.length">
          ...
          <!-- i18n:['state-temperature','state-humidity','state-low','state-hi','state-half'] -->
          <h5 v-if="possibleStates.length > 1" class="no-margin-top">
            {{ $t(`state-${possibleState}`) }}
          </h5>
          <div class="dropbox">
            <input type="file" multiple accept="image/*" class="input-file" :disabled="uploading" @change="onFileChosen($event.target.files, stateIndex)" />
            <img v-if="previews[stateIndex]" :src="previews[stateIndex]" class="icon-preview" />
            <p v-else>{{ $t('Drag your image(s) here or click to select from the disk') }}</p>
          </div>
        </div>
      </div>
    </div>
    <div v-show="mode === 'dark'" class="form-group">
      <div class="row">
        <div v-for="(possibleState, stateIndex) in possibleStates" :key="stateIndex" :class="'col-sm-' + 12 / possibleStates.length">
          <h5 v-if="possibleStates.length > 1" class="no-margin-top">
            {{ $t(`state-${possibleState}`) }}
          ...
          <div class="dropbox">
            ...
            />
            <img v-if="previewsDark[stateIndex]" :src="previewsDark[stateIndex]" alt="" class="icon-preview" />
            <p v-else>{{ $t('Drag your image(s) here or click to select from the disk') }}</p>
          </div>
        </div>
      </div>
    ...
    <label class="checkbox2 text-left">
      <input v-model="fileCopyrightConfirmed" type="checkbox" />
      {{
    ...
    <p v-if="filesTooBig" class="text-danger">
      {{ $t('The set of icons you have chosen is too large. Maximum upload limit is {limit}.', {limit: maxUploadSizeTotalPretty}) }}
    </p>
    <div class="row">
      <div class="col-xs-12">
        <button class="btn btn-green mx-2" type="button" :disabled="!fileCopyrightConfirmed || filesTooBig" @click="uploadIcons()">
          {{ $t(icon ? 'Save' : 'Add') }}
        </button>
        <a v-if="icon" class="btn btn-red mx-2" @click="deleteConfirm = true">
          {{ $t('Delete') }}
        </a>
        <a class="btn btn-white mx-2" @click="$emit('cancel')">
          {{ $t('Cancel') }}
        </a>
      </div>
    </div>
  ...
  export default {
    ...
    methods: {
      ...
      deleteIcon() {
        this.uploading = true;
        api
          .delete_('user-icons/' + this.icon.id)
