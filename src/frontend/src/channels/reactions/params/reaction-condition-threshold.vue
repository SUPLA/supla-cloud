<template>
  <div class="reaction-condition-threshold">
    ...
    <div v-if="resumeOperator" class="form-group d-flex align-items-center">
      ...
      <span class="input-group">
        ...
        <span v-if="unitBefore(field, subject)" class="input-group-addon">{{ $t(unitBefore(field, subject)) }}</span>
        <span v-if="valuesForDropdown.length" class="input-group-addon">
          {{ threshold }}
        </span>
        <input
          v-else
          v-model="resumeThreshold"
          type="number"
          required
          class="form-control"
          :step="step()"
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      validValues() {
        return (
          Number.isFinite(parseFloat(this.threshold)) &&
