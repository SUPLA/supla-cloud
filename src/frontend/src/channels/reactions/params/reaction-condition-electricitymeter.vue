<template>
  <div class="reaction-condition-em">
    ...
    <transition-expand>
      <div v-if="subject.config.enabledPhases.length > 1 && !field.disablePhases" class="form-group d-flex align-items-center">
        <label class="flex-grow-1 pr-3">{{ $t('Phase') }}</label>
        <div class="btn-group btn-group-flex">
          <a v-for="p in availablePhases" :key="p" :class="'btn ' + (phase === p ? 'btn-green' : 'btn-default')" @click="phase = p">
            {{ p === 'all' ? (field.name === 'voltage' ? $t('average') : $t('sum')) : p }}
          </a>
        </div>
      </div>
    </transition-expand>
    <ReactionConditionThreshold
      v-if="field"
      v-model="trigger"
      :field="fieldName"
      :subject="subject"
      ...
      :resume-label-i18n="resumeLabelI18n"
    />
  </div>
</template>
