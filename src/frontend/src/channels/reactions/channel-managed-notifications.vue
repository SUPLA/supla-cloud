    ...
    <loading-cover :loading="notification === undefined">
      <div v-if="notification">
        <p class="mb-5">
          {{
            $t(
              'Firmware of this device sends notifications in context of this channel. You can configure its recipients and content here. For more information consult the firmware issuer.'
            )
          }}
        </p>
        <div class="row">
...
<script>
  ...
  export default {
    ...
    mounted() {
      api
        .get(`channels/${this.subject.id}/notifications?onlyManaged=true&include=accessIds`)
        .then((response) => {
