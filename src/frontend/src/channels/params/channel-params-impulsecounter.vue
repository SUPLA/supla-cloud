<template>
  <div>
    <dl>
      <dd>{{ $t('Unit') }}</dd>
      <dt>
        <input v-model="channel.config.unit" type="text" maxlength="8" class="form-control text-center" @input="$emit('change')" />
      </dt>
      <channel-params-meter-cost :channel="channel" :unit="unit" @change="$emit('change')"></channel-params-meter-cost>
      <dd>{{ $t('Impulses') }}</dd>
      <dt>
      ...
      <dt>
        ...
        <channel-params-meter-initial-values-mode
          v-model="channel.config.addToHistory"
          class="small"
          @input="$emit('change')"
        ></channel-params-meter-initial-values-mode>
      </dt>
...
<script>
  ...
  export default {
    components: {
      ...
      ChannelParamsMeterReset,
      ChannelsIdDropdown,
      ChannelParamsMeterCost,
    },
    props: ['channel'],
    computed: {
      unit() {
