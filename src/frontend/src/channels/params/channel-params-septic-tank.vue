<template>
  <div>
          ...
          <div v-for="lvl in levelSensorsDef" :key="lvl.id" class="d-flex align-items-center bottom-border py-2">
            <div class="flex-grow-1">
              ...
              <dl>
                <dt>{{ $t('Fill level') }}</dt>
                <dd>
                  <NumberInput v-model="lvl.fillLevel" :min="1" :max="100" suffix=" %" class="form-control text-center mt-2" @input="levelChanged()" />
                </dd>
              </dl>
            </div>
            <div class="pl-3">
    ...
    <dl v-for="alarm in availableAlarms" :key="alarm">
      ...
      <dt>
        <div v-if="fillLevelInFullRange" class="d-flex align-items-center ml-2">
          <label class="checkbox2 checkbox2-grey">
            <input
              type="checkbox"
              :checked="channel.config[alarm] !== null"
              @change="
                channel.config[alarm] = channel.config[alarm] === null ? 0 : null;
                emit('change');
              "
            />
          </label>
        ...
        <SimpleDropdown
          v-else-if="availableFillLevelsPerAlarm[alarm].length > 0"
          v-model="channel.config[alarm]"
    ...
    <dl>
      ...
      <dt class="text-center">
        <Toggler v-model="channel.config.muteAlarmSoundWithoutAdditionalAuth" @update:model-value="$emit('change')" />
      </dt>
    </dl>
  </div>
</template>
...
<script setup>
  ...
  const availableFillLevelsPerAlarm = computed(() => ({
    alarmAboveLevel: availableFillLevels.value.filter(
      (fl) => fl > (props.channel.config.warningAboveLevel || props.channel.config.warningBelowLevel || props.channel.config.alarmBelowLevel || 0)
    ),
    warningAboveLevel: availableFillLevels.value.filter(
      (fl) => fl > (props.channel.config.warningBelowLevel || props.channel.config.alarmBelowLevel || 0) && fl < (props.channel.config.alarmAboveLevel || 101)
    ),
    warningBelowLevel: availableFillLevels.value
      .filter(
        (fl) =>
          fl !== props.channel.config.alarmBelowLevel &&
          fl > (props.channel.config.alarmBelowLevel || -1) &&
          fl <
            (props.channel.config.warningAboveLevel || props.channel.config.alarmAboveLevel || availableFillLevels.value[availableFillLevels.value.length - 1])
      )
      .reverse(),
    alarmBelowLevel: availableFillLevels.value
      .filter(
        (fl) =>
          fl !== props.channel.config.warningBelowLevel &&
          fl <
            (props.channel.config.warningBelowLevel ||
              props.channel.config.warningAboveLevel ||
              props.channel.config.alarmAboveLevel ||
              availableFillLevels.value[availableFillLevels.value.length - 1])
      )
      .reverse(),
  }));
  ...
  function onlyFreeSensors(sensor) {
    return dependenciesStore.forChannel(sensor.id).filter((dep) => dep.role === 'levelSensorChannelIds').length === 0;
  }
