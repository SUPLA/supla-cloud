<template>
  <modal-confirm class="modal-warning" :loading="loading" :header="$t(headerI18n)" @confirm="$emit('confirm')" @cancel="$emit('cancel')">
    <p v-if="descriptionI18n">{{ $t(descriptionI18n) }}</p>
    <subject-dependencies :dependencies="dependencies.dependencies">
      <template #deletingHeader>
        {{ $t(deletingHeaderI18n) }}
      </template>
      <template #removingHeader>
