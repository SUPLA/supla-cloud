<template>
  <div>
    ...
    <div class="row">
      <div v-if="dependentSchedules.length > 0" class="col-sm-6 col-12-if-alone">
        <h5>{{ $t('Schedules') }}</h5>
        <ul>
          <li v-for="schedule in dependentSchedules" :key="schedule.id">
            ID{{ schedule.id }}
            <span class="small">{{ schedule.caption }}</span>
          </li>
        </ul>
      </div>
      <div v-if="dependentDirectLinks.length > 0" class="col-sm-6 col-12-if-alone">
        <h5>{{ $t('Direct links') }}</h5>
        <ul>
          <li v-for="link in dependentDirectLinks" :key="link.id">
            ID{{ link.id }}
            <span class="small">{{ link.caption }}</span>
          </li>
        </ul>
      </div>
      <div v-if="dependentReactions.length > 0 || dependentOwnReactions.length > 0" class="col-sm-6 col-12-if-alone">
        <h5>{{ $t('Reactions') }}</h5>
        <ul>
          <li v-for="reaction in dependentReactions" :key="reaction.id + 'dep'">
          ...
          <li v-for="reaction in dependentOwnReactions" :key="reaction.id + 'owndep'">
            ID{{ reaction.owningChannelId }}
            <span class="small">
              {{ humanizeTrigger(reaction) }}
              &raquo;
    ...
    <div class="row">
      <div v-if="(dependentChannels || []).length > 0" class="col-sm-6 col-12-if-alone">
        <h5>{{ $t('Channels') }}</h5>
        <ul>
      ...
      <div v-if="dependentChannelGroups.length > 0" class="col-sm-6 col-12-if-alone">
        <h5>{{ $t('Channel groups') }}</h5>
        <ul>
          <li v-for="group in dependentChannelGroups" :key="group.id">
            ID{{ group.id }}
    ...
    <div class="row">
      <div v-if="dependentScenes.length > 0" class="col-sm-6 col-12-if-alone">
        <h5>{{ $t('Scenes') }}</h5>
        <ul>
          <li v-for="scene in dependentScenes" :key="scene.id">
            ID{{ scene.id }}
            <span class="small">{{ scene.caption }}</span>
          </li>
        </ul>
      </div>
      <div v-if="dependentActionTriggers.length > 0" class="col-sm-6 col-12-if-alone">
        <h5>{{ $t('Action triggers') }}</h5>
        <ul>
          <li v-for="channel in dependentActionTriggers" :key="channel.id">
            ID{{ channel.id }}
