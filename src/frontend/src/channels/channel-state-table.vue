<template>
  <div v-if="currentState" class="channel-state-table">
    ...
    <div v-if="!currentState.isCalibrating && !currentState.notCalibrated">
      ...
      <div v-if="currentState.tiltPercent > -1">
        <dl>
      ...
      <dl v-else-if="currentState.tiltPercent === -1">
        <dd>{{ $t('Not calibrated') }}</dd>
        <dt></dt>
      </dl>
    </div>
    ...
    <dl v-if="currentState.calculatedValue !== undefined">
      <dd>{{ $t('Meter value') }}</dd>
      <dt>{{ roundToDecimals(currentState.calculatedValue) }} {{ currentState.unit || '' }}</dt>
    </dl>
    <dl v-if="currentState.phases && currentState.phases[0].totalForwardActiveEnergy !== undefined">
      ...
      <dt>
        {{
          roundToDecimals(
            currentState.phases[0].totalForwardActiveEnergy + currentState.phases[1].totalForwardActiveEnergy + currentState.phases[2].totalForwardActiveEnergy
          )
        }}
        kWh
      </dt>
    ...
    <strong v-if="currentState.value !== undefined">
      ...
      <span v-else-if="channel.function.name === 'WEIGHTSENSOR'">
        <span v-if="currentState.value >= 2000"> {{ roundToDecimals(currentState.value / 1000, 4) }} kg </span>
        <span v-else> {{ currentState.value }} g </span>
      </span>
      <span v-else-if="channel.function.name === 'RAINSENSOR'"> {{ roundToDecimals(currentState.value / 1000, 4) }} mm </span>
