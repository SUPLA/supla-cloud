<template>
  <div :class="['channel-action-chooser', `channel-action-chooser-action-${action && action.id}`]">
    <div class="w-100">
      <div v-if="isSlaveThermostat" class="alert alert-info">
      ...
      <div :class="['panel-group panel-accordion m-0', {'panel-accordion-disabled': isDisabled}]">
        <div
          ...
          :class="[
            {
              'panel panel-default': possibleAction.name !== 'SEND',
              'panel-success': isSelected(possibleAction.id),
              'action-without-params': !ChannelFunctionAction.requiresParams(possibleAction.id),
            },
          ]"
        >
          <div v-show="possibleAction.name !== 'SEND'" class="panel-heading" @click="changeAction(possibleAction)">
            <a role="button" tabindex="0" class="text-inherit" @keydown.enter.stop="changeAction(possibleAction)">
          ...
          <transition-expand>
            <div
              ...
              :class="{'panel-body': possibleAction.name !== 'SEND'}"
            >
              <div
                v-if="
                  [
              ...
              <div v-if="action.id === ChannelFunctionAction.SET">
                <digiglass-parameters-setter
                  v-if="subject.function.name.match(/^DIGIGLASS.+/)"
                  v-model="param"
                  :subject="subject"
                  @input="paramsChanged()"
                ></digiglass-parameters-setter>
              ...
              <div v-if="action.id === ChannelFunctionAction.COPY">
                <channels-id-dropdown
                  v-model="param.sourceChannelId"
                  :hide-none="true"
                  :params="`function=${subject.function.id}&skipIds=${(subject.ownSubjectType === 'channel' && subject.id) || ''}`"
                  @input="paramsChanged()"
                ></channels-id-dropdown>
              </div>
              <div v-if="action.id === ChannelFunctionAction.TURN_OFF_WITH_DURATION">
                <DurationParamSetter v-model="param.durationMs" :with-calendar="executorMode" hide-no-timer disable-ms @input="paramsChanged()" />
              </div>
              <div v-if="action.id === ChannelFunctionAction.HVAC_SET_TEMPERATURES || action.id === ChannelFunctionAction.HVAC_SET_TEMPERATURE">
              ...
              <div v-if="action.id === ChannelFunctionAction.SEND">
                <NotificationForm v-model="param" display-validation-errors :subject="contextSubject" @input="paramsChanged()" />
              </div>
