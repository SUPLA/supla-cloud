  ...
  <div>
    <div class="form-group">
      ...
      <span class="input-group">
        <NumberDeltaInput v-model="percentage" :min="-100" :max="100" @input="onChange()" />
        <span class="input-group-addon">%</span>
      </span>
    </div>
    <div v-if="hasTilt" class="form-group">
      <label v-if="isDelta(tilt)">{{ $t('Change tilt by') }}</label>
      <label v-else>{{ $t('Change tilt to') }}</label>
      <span class="input-group">
        <NumberDeltaInput v-model="tilt" :min="-100" :max="100" @input="onChange()" />
        <span class="input-group-addon">%</span>
      </span>
    </div>
    <div class="small">
