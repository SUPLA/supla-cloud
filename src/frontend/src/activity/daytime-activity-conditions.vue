  ...
  <div>
    ...
    <div>
      <div v-for="(t, $index) in times" :key="t.id">
        <div>
          <DaytimeActivityCondition v-model="t.times" class="flex-grow-1" @update:model-value="updateModel()" />
        ...
        <div v-if="$index < times.length - 1" class="or-hr">{{ $t('OR') }}</div>
      </div>
    </div>
    <a v-if="times.length < 2" class="btn btn-white" @click="addTime()">
      <fa icon="plus" />
      {{ $t('Add') }}
    </a>
  </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      initTimesFromModel() {
        this.times = this.conditionsToTimes(this.value || []).map((times) => ({
          id: ++this.timeIdCounter,
          times,
        }));
      },
