        ...
        <transition name="fade-router">
          <div v-if="!id && !app">
            <div v-if="filteredApps.length">
              <square-links-grid :count="filteredApps.length">
        ...
        <transition name="fade-router">
          <div v-if="app">
            <div class="container form-group">
              <router-link :to="{name: 'publicApps'}"> &laquo; {{ $t('Back to apps list') }} </router-link>
            </div>
            <public-app-details :app="app"></public-app-details>
          </div>
        </transition>
      </div>
      <div v-if="apps && !apps.length" class="container">
...
<script>
  ...
  export default {
    components: {
      ...
      PublicAppDetails,
      PublicAppTile,
      PublicAppsCatalogNotRegisteredMessage,
    },
    props: ['id'],
    data() {
      return {
    ...
    mounted() {
      api
        .get('public-oauth-apps', {skipErrorHandler: [406]})
