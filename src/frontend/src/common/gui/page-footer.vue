      ...
      <div class="row">
        ...
        <div class="col-sm-3 text-right">
          ...
          <div v-else-if="isLoginPage && !frontendConfigStore.config.maintenanceMode">
            <a v-if="showRegisterCloud" class="brand nav-link" :href="`https://cloud.supla.org/register-cloud?domain=${domain}`">
              {{ $t('Register your SUPLA Cloud') }}
            </a>
          </div>
          <a v-else :href="$router.resolve({name: 'login'}).href" class="nav-link">
            {{ $t('Sign In') }}
          </a>
        </div>
      </div>
