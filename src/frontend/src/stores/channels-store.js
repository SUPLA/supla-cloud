...
export const useChannelsStore = defineStore('channels', () => {
  ...
  const fetchChannel = (channelId) => {
    return channelsApi.getOneWithState(channelId).then((channel) => {
  ...
  const fetchStates = () => {
    if (!fetchStates.promise) {
      fetchStates.promise = channelsApi
        .getStates()
        .then((response) => {
          let refetch = false;
          const idsToFetch = [];
  ...
  const filteredChannels = computed(() => (filters) => {
    ...
    params.deviceIds = ('' + (params.deviceIds || ''))
      .split(',')
      .filter((id) => !!id)
      .map((id) => +id);
    params.skipIds = ('' + (params.skipIds || ''))
      .split(',')
      .filter((id) => !!id)
      .map((id) => +id);
    params.type = ('' + (params.type || ''))
      .split(',')
      .filter((t) => !!t)
      .map((t) => ChannelType[t] || +t);
    params.function = ('' + (params.function || params.fnc || ''))
      .split(',')
      .filter((t) => !!t)
      .map((t) => ChannelFunction[t] || +t);
