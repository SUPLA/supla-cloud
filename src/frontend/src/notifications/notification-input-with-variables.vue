<template>
  <div>
    <label class="d-flex">
      ...
      <a v-if="!disabled" class="ml-2 text-normal" @click="helpShown = !helpShown">
        {{ $t('List of available variables') }}
        <fa icon="question-circle" class="ml-1" />
      </a>
    </label>
    <transition-expand>
      <div v-if="helpShown" class="variable-field-help">
        <div class="d-flex">
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      modelHighlighted() {
        const highlighted = this.model.replace(new RegExp('&', 'g'), '&amp;').replace(new RegExp('<', 'g'), '&lt;');
        return this.allVariables.reduce((h, {value}) => h.replace(new RegExp('{' + value, 'g'), `<span class="variable">{${value}</span>`), highlighted);
      },
      subjectVariables() {
