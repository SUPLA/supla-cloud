<template>
  <square-link :class="'clearfix pointer with-label ' + (model.enabled ? 'green' : 'grey')" @click="$emit('click')">
    <h3>{{ model.caption }}</h3>
    <dl>
    ...
    <dl>
      <dd>{{ $t('Access ID') }}</dd>
      <dt>{{ model.accessId ? model.accessId.caption : $t('None') }}</dt>
    </dl>
    <div class="square-link-label">
      <client-app-connection-status-label :app="model"></client-app-connection-status-label>
    </div>
  </square-link>
</template>
