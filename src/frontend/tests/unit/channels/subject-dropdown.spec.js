...
describe('SubjectDropdown', () => {
  ...
  const possibleAction = (id) => ({
    ...
    caption: Object.keys(ChannelFunctionAction)
      .find((k) => ChannelFunctionAction[k] === id)
      .replace(/_/g, ' '),
  });
  ...
  it('selects channel action', async () => {
    const channel = channelStub(ChannelFunction.CONTROLLINGTHEGARAGEDOOR, {
      possibleActions: [
        possibleAction(ChannelFunctionAction.OPEN_CLOSE),
        possibleAction(ChannelFunctionAction.OPEN),
        possibleAction(ChannelFunctionAction.CLOSE),
      ],
    });
    ...
    const channels = [
      channelStub(ChannelFunction.CONTROLLINGTHEROLLERSHUTTER, {
        possibleActions: [
          possibleAction(ChannelFunctionAction.OPEN),
          possibleAction(ChannelFunctionAction.CLOSE),
          possibleAction(ChannelFunctionAction.CLOSE_PARTIALLY),
        ],
      }),
    ];
