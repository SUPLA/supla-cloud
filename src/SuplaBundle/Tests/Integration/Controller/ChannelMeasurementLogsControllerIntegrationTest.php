...
class ChannelMeasurementLogsControllerIntegrationTest extends IntegrationTestCase {
    ...
    private function testGettingMeasurementLogsCount(int $channelId) {
    ...
    private function testGettingMeasurementLogs(int $channelId) {
        $client = $this->createAuthenticatedClient($this->user);
        $client->apiRequestV22('GET', '/api/channels/'.$channelId.'/measurement-logs');
        $response = $client->getResponse();
        $this->assertStatusCode('2xx', $response);
    ...
    private function testElectricityMeasurementLogsOrder(array $content, array $energiesInOrder) {
        foreach ([1,2,3] as $phase) {
    ...
    public function testGettingTemperatureLogs() {
        $content = $this->testGettingMeasurementLogs(2);
    ...
    public function testGettingTemperatureAndHumidityLogs() {
        $content = $this->testGettingMeasurementLogs(3);
        $temperaturesInOrder = [23, 22, 21];
        $this->assertEquals($temperaturesInOrder, array_map('intval', array_column($content, 'temperature')));
    ...
    public function testGettingElectricityMeasurementLogs() {
        $content = $this->testGettingMeasurementLogs(4);
        $this->testElectricityMeasurementLogsOrder($content, [855000,854900,854800]);
    }
    
    private function testGettingMeasurementLogsAscending(int $channelId) {
        $client = $this->createAuthenticatedClient($this->user);
    ...
    public function testGettingTemperatureLogsAscending() {
        $content = $this->testGettingMeasurementLogsAscending(2);
        $temperaturesInOrder = [21, 22, 23];
        $this->assertEquals($temperaturesInOrder, array_map('intval', array_column($content, 'temperature')));
    }
    
    public function testGettingTemperatureAndHumidityLogsAscending() {
        $content = $this->testGettingMeasurementLogsAscending(3);
        $temperaturesInOrder = [21, 22, 23];
        $this->assertEquals($temperaturesInOrder, array_map('intval', array_column($content, 'temperature')));
    ...
    public function testGettingElectricityMeasurementLogsAscending() {
        $content = $this->testGettingMeasurementLogsAscending(4);
        $this->testElectricityMeasurementLogsOrder($content, [854800,854900,855000]);
    }

    private function testGettingMeasurementLogsWithOffset(int $channelId) {
        $client = $this->createAuthenticatedClient($this->user);
    ...
    public function testGettingTemperatureLogsWithOffset() {
        $content = $this->testGettingMeasurementLogsWithOffset(2);
        $this->assertEquals(22, intval($content[0]['temperature']));
    }
    
    public function testGettingTemperatureAndHumidityLogsWithOffset() {
        $content = $this->testGettingMeasurementLogsWithOffset(3);
        $this->assertEquals(22, intval($content[0]['temperature']));
        $this->assertEquals(40, intval($content[0]['humidity']));
    }
    
    public function testGettingElectricityMeasurementLogsWithOffset() {
        $content = $this->testGettingMeasurementLogsWithOffset(4);
        $this->testElectricityMeasurementLogsOrder($content, [854900]);
    }
    
    private function testGettingMeasurementLogsWithTimestampRange(int $channelId) {
    ...
    public function testGettingTemperatureLogsWithTimestampRange() {
        $content = $this->testGettingMeasurementLogsWithTimestampRange(2);
        $this->assertEquals(22, intval($content[0]['temperature']));
    }
    
    public function testGettingTemperatureAndHumidityLogsWithTimestampRange() {
        $content = $this->testGettingMeasurementLogsWithTimestampRange(3);
        $this->assertEquals(22, intval($content[0]['temperature']));
        $this->assertEquals(40, intval($content[0]['humidity']));
    }
    
    public function testGettingElectricityMeasurementLogsWithTimestampRange() {
        $content = $this->testGettingMeasurementLogsWithTimestampRange(4);
        $this->testElectricityMeasurementLogsOrder($content, [854900]);
    }
