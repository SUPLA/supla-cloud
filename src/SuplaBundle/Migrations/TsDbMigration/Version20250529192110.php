...
class Version20250529192110 extends NoWayBackMigration {
    public function migrate() {
        ...
        $this->addSql('CREATE TABLE supla_em_voltage_log (channel_id INT NOT NULL, date TIMESTAMP(0) WITHOUT TIME ZONE NOT NULL, phase_no SMALLINT NOT NULL, min NUMERIC(5, 2) NOT NULL, max NUMERIC(5, 2) NOT NULL, avg NUMERIC(5, 2) NOT NULL, PRIMARY KEY(channel_id, date, phase_no)) WITH (tsdb.hypertable, tsdb.partition_column=\'date\')');
        ...
        $this->addSql('CREATE TABLE supla_gp_measurement_log (channel_id INT NOT NULL, date TIMESTAMP(0) WITHOUT TIME ZONE NOT NULL, open_value DOUBLE PRECISION NOT NULL, close_value DOUBLE PRECISION NOT NULL, avg_value DOUBLE PRECISION NOT NULL, max_value DOUBLE PRECISION NOT NULL, min_value DOUBLE PRECISION NOT NULL, PRIMARY KEY(channel_id, date)) WITH (tsdb.hypertable, tsdb.partition_column=\'date\')');
        $this->addSql('COMMENT ON COLUMN supla_gp_measurement_log.date IS \'(DC2Type:stringdatetime)\'');
        $this->addSql('CREATE TABLE supla_gp_meter_log (channel_id INT NOT NULL, date TIMESTAMP(0) WITHOUT TIME ZONE NOT NULL, value DOUBLE PRECISION NOT NULL, PRIMARY KEY(channel_id, date)) WITH (tsdb.hypertable, tsdb.partition_column=\'date\')');
        $this->addSql('COMMENT ON COLUMN supla_gp_meter_log.date IS \'(DC2Type:stringdatetime)\'');
        $this->addSql('CREATE TABLE supla_ic_log (channel_id INT NOT NULL, date TIMESTAMP(0) WITHOUT TIME ZONE NOT NULL, counter BIGINT NOT NULL, calculated_value BIGINT NOT NULL, PRIMARY KEY(channel_id, date)) WITH (tsdb.hypertable, tsdb.partition_column=\'date\')');
        $this->addSql('COMMENT ON COLUMN supla_ic_log.date IS \'(DC2Type:stringdatetime)\'');
        $this->addSql('CREATE TABLE supla_temperature_log (channel_id INT NOT NULL, date TIMESTAMP(0) WITHOUT TIME ZONE NOT NULL, temperature NUMERIC(8, 4) NOT NULL, PRIMARY KEY(channel_id, date)) WITH (tsdb.hypertable, tsdb.partition_column=\'date\')');
        $this->addSql('COMMENT ON COLUMN supla_temperature_log.date IS \'(DC2Type:stringdatetime)\'');
        $this->addSql('CREATE TABLE supla_temphumidity_log (channel_id INT NOT NULL, date TIMESTAMP(0) WITHOUT TIME ZONE NOT NULL, temperature NUMERIC(8, 4) NOT NULL, humidity NUMERIC(8, 4) NOT NULL, PRIMARY KEY(channel_id, date)) WITH (tsdb.hypertable, tsdb.partition_column=\'date\')');
