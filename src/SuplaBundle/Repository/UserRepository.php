...
class UserRepository extends EntityWithRelationsRepository {
    ...
    protected function getEntityWithRelationsCountQuery(): QueryBuilder {
        return $this->_em->createQueryBuilder()
            ...
            ->addSelect(sprintf('(SELECT COUNT(1) FROM %s c WHERE c.user = u AND c.type != %d) channels', IODeviceChannel::class, ChannelType::VIRTUAL))
            ->addSelect(sprintf('(SELECT COUNT(1) FROM %s cv WHERE cv.user = u AND cv.type = %d) virtualChannels', IODeviceChannel::class, ChannelType::VIRTUAL))
