...
namespace App\Model\Schedule\SchedulePlanners;
...
class CompositeSchedulePlanner {
        ...
        return CompositeSchedulePlanner::wrapInScheduleTimezone($schedule, function () use ($schedule, $currentDate) {
            ...
            foreach ($schedule->getConfig() as $executionDef) {
                ...
                if (!$closestExecution || $closestExecution->getPlannedTimestamp() > $nextRunDate) {
                    $action = $executionDef['action']['id'] ?? false ? new ChannelFunctionAction($executionDef['action']['id']) : null;
                    $closestExecution = new ScheduledExecution($schedule, $nextRunDate, $action, $executionDef['action']['param'] ?? null);
                }
            }
     ...
     * @param string $until
     * @param int $maxCount
     * @return \App\Entity\Main\ScheduledExecution[]
     */
