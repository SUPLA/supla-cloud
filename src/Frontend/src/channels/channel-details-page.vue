    ...
    export default {
        ...
        methods: {
            fetchChannel() {
                this.loading = true;
                this.error = false;
                this.$http.get(`channels/${this.id}?include=iodevice,channel.location,location.childrenIds,iodevice.location,supportedFunctions,relationsCount`, {skipErrorHandler: [403, 404]})
                    .then(response => {
