...
use App\Exception\DirectLinkExecutionFailureException;
use App\Exception\SceneDuringExecutionException;
use App\Exception\SceneDuringInactivePeriodException;
...
class ExecuteDirectLinkController extends AbstractController {
    use Transactional;
        ...
        try {
            ...
            if ($directLink) {
                if ($directLink->getSubjectType() == ActionableSubjectType::CHANNEL()) {
                    ...
                    /** @var \App\Entity\Main\IODeviceChannelGroup $channelGroup */
    ...
    private function ensureLinkCanBeUsed(Request $request, DirectLink $directLink, string $slug) {
        ...
        if (!$directLink->isValidSlug($slug, $this->passwordHasherFactory->getPasswordHasher($directLink))) {
            throw new DirectLinkExecutionFailureException(DirectLinkExecutionFailureReason::INVALID_SLUG(), [], Response::HTTP_FORBIDDEN);
        }
    ...
    /**
     * @param \App\Entity\Main\DirectLink|null $directLink
     */
