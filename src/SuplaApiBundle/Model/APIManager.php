...
class APIManager 
{
	...
	public function getAPIUser(ParentUser $parent) {
		...
		$api_user = $this->oauth_user_rep->findOneBy(array('parent' => $parent, 'accessId' => null));
	...
	public function getAPIUserByName($username) {
		...
		return $this->oauth_user_rep->findOneBy(array('id' => intval($username)));
	}
	...
	public function deleteTokens(APIUser $user) {
		...
		->delete()
		->where('t.user = ?1')
		->setParameters(array(1 => $user->getId()));
	...
	public function userLogout(APIUser $user, $accessToken, $refreshToken) {
		...
		->setParameters(array(1 => $user->getId(), 2 => $accessToken));
		...
		->delete()
		->where('t.user = ?1 AND t.token = ?2')
		->setParameters(array(1 => $user->getId(), 2 => $refreshToken));
